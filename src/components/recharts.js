...
const DataDisplay = () =>{
    ...
    useEffect(() => {
        ...
        const data_url = rechartsObj.site_url + "wp-json/recharts/v1/data";
        const now = new Date();
    ...
    if (error) {
        return <div>{__('Error:', 'recharts')} {error.message}</div>;
    }

    if(!data.length){
        return(
            <div>
                <p>{__('No data available!', 'recharts')}</p>
            </div>
        );
    }
    ...
    return (
        <div>
            <div className="graph_period">
                <div className="widget_tite">
                    <h2>{__('Graph Widget', 'recharts')}</h2>
                </div>
                <div className="widget_select">
                    <select id="period" name="period" value={period} onChange={handlePeriodChange}>
